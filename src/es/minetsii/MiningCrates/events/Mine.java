...
public class Mine implements Listener {
	...
	public void mineEvent(BlockBreakEvent e) {
		...
		if (!MiningCrates.blocksAffected.containsKey(e.getBlock().getType()))
			return;
		if (!MiningCrates.placedBlocks.contains(e.getBlock().getLocation()))
			return;
		for (String group : MiningCrates.groups.keySet()) {
			try {
				...
				if (i > probability
						|| detectchest(e.getBlock().getLocation()) == false)
					break;
						...
						new BukkitRunnable() {
							public void run() {
								for (Player pl : Bukkit.getOnlinePlayers()) {
									if (p != pl)
										pl.sendBlockChange(b.getLocation(),
												Material.AIR, (byte) 0);
								}
							}
	...
	public void chestOpen(PlayerInteractEvent e) {
		...
		if (e.getAction().equals(Action.RIGHT_CLICK_BLOCK)) {
			...
			if (m.equals(Material.TRAPPED_CHEST)) {
				...
				if (ci.getItem(0) == null || ci.getItem(1) == null)
					return;
				if (!ci.getItem(0).getType()
						.equals(Material.ENDER_PORTAL_FRAME)
	...
	public void destroy(BlockBreakEvent e) {
		...
		if (m.equals(Material.CHEST)) {
			...
			if (!ci.getItem(0).getType().equals(Material.ENDER_PORTAL_FRAME)
					|| !ci.getItem(1).getType().equals(Material.DRAGON_EGG))
				return;
			if (!e.getPlayer().hasPermission("MiningCrates.destroy"))
				e.setCancelled(true);
			else
				ci.clear();
		}
	...
	public void spawn(ItemSpawnEvent e) {
		...
		if (it.getItemStack().getType().equals(Material.DRAGON_EGG)) {
			for (Player p : Bukkit.getOnlinePlayers()) {
				if (it.getItemStack().getItemMeta().getDisplayName() == null)
					return;
				if (it.getItemStack().getItemMeta().getDisplayName()
		...
		if (it.getItemStack().getType().equals(Material.ENDER_PORTAL_FRAME)) {
			for (Crate c : MiningCrates.chestList.keySet()) {
				...
				if (c.getName().equalsIgnoreCase(
						it.getItemStack().getItemMeta().getDisplayName()))
					it.remove();
